...
<body class="h-full bg-slate-50 text-slate-900 flex items-center justify-center p-4 selection:bg-primary-500 selection:text-white">
    <div class="w-full max-w-md space-y-6">
        
        <!-- Header Branding -->
        <div class="text-center space-y-3">
            @if(App\Models\Setting::get('website_logo'))
                <img class="h-16 w-auto mx-auto drop-shadow-xs" src="{{ asset('storage/' . App\Models\Setting::get('website_logo')) }}" alt="Logo">
            @else
                <div class="w-14 h-14 rounded-2xl bg-gradient-to-tr from-primary-600 to-primary-500 flex items-center justify-center text-white font-black text-2xl shadow-md shadow-primary-500/30 mx-auto border border-white">
                    S
                </div>
            @endif

            <div class="space-y-1">
                <div class="inline-flex items-center gap-1.5 px-3 py-1 rounded-full bg-primary-50 border border-primary-200/80 text-primary-700 text-[10px] font-extrabold uppercase tracking-widest">
        ...
        <div class="bg-white border border-slate-200/80 rounded-2xl p-6 sm:p-8 shadow-xl shadow-slate-200/50 space-y-5">
            @if(session('success'))
                <div class="p-3.5 bg-emerald-50 border border-emerald-200/80 rounded-xl text-emerald-800 text-xs font-bold flex items-center gap-2">
                    <span class="text-emerald-600">✓</span> {{ session('success') }}
                </div>
            @endif

            @if($errors->any())
                <div class="p-3.5 bg-rose-50 border border-rose-200/80 rounded-xl text-rose-800 text-xs font-semibold space-y-1">
            ...
            <form method="POST" action="{{ route('admin.login.submit') }}" class="space-y-4">
                ...
                <!-- Password -->
                <div class="space-y-1.5">
                    <label for="password" class="block text-[11px] font-extrabold uppercase tracking-wider text-slate-600">
                ...
                <div class="flex items-center justify-between pt-1">
                    <label class="flex items-center gap-2 cursor-pointer select-none">
                        <input type="checkbox" name="remember" class="w-4 h-4 rounded border-slate-300 bg-slate-50 text-primary-600 focus:ring-0 focus:ring-offset-0">
        ...
        <div class="text-center pt-2">
            <a href="{{ route('home') }}" class="text-xs font-bold text-slate-500 hover:text-slate-900 transition-colors inline-flex items-center gap-1.5">
