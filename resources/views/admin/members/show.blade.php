            ...
            <div class="flex-1 w-full space-y-4">
                ...
                @if($member->memberProfile)
                    <div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-x-6 gap-y-3 text-xs text-slate-700">
                        <div>
                            <h5 class="text-[10px] font-extrabold text-slate-400 uppercase tracking-wider mb-0.5">{{ __('messages.full_name') }}</h5>
                            <p class="font-bold text-slate-900 text-xs">{{ $member->memberProfile->first_name }} {{ $member->memberProfile->middle_name }} {{ $member->memberProfile->last_name }}</p>
                        </div>
                        <div>
                            <h5 class="text-[10px] font-extrabold text-slate-400 uppercase tracking-wider mb-0.5">{{ __('messages.gender') }}</h5>
                            <p class="font-bold text-slate-900 text-xs">{{ $member->memberProfile->gender }}</p>
                        </div>
                        <div>
                            <h5 class="text-[10px] font-extrabold text-slate-400 uppercase tracking-wider mb-0.5">{{ __('messages.dob') }}</h5>
                            <p class="font-bold text-slate-900 text-xs">{{ date('d-M-Y', strtotime($member->memberProfile->dob)) }}</p>
                        </div>
                        <div>
                            <h5 class="text-[10px] font-extrabold text-slate-400 uppercase tracking-wider mb-0.5">{{ __('messages.phone_whatsapp') }}</h5>
                            <p class="font-bold text-slate-900 text-xs">
                                {{ $member->memberProfile->phone }} 
                                @if($member->memberProfile->whatsapp)
                                    <span class="text-slate-400 font-medium text-[11px]">/ {{ $member->memberProfile->whatsapp }}</span>
                                @endif
                            </p>
                        </div>
                        <div>
                            <h5 class="text-[10px] font-extrabold text-slate-400 uppercase tracking-wider mb-0.5">{{ __('messages.blood_group') }}</h5>
                            <p class="font-bold text-slate-900 text-xs">{{ $member->memberProfile->blood_group ?? __('messages.not_declared') }}</p>
                        </div>
                        <div>
                            <h5 class="text-[10px] font-extrabold text-slate-400 uppercase tracking-wider mb-0.5">{{ __('messages.education') }}</h5>
                            <p class="font-bold text-slate-900 text-xs">{{ $member->memberProfile->education ?? __('messages.not_declared') }}</p>
                        </div>
                        <div>
                            <h5 class="text-[10px] font-extrabold text-slate-400 uppercase tracking-wider mb-0.5">{{ __('messages.occupation') }}</h5>
                            <p class="font-bold text-slate-900 text-xs">{{ $member->memberProfile->occupation ?? __('messages.not_declared') }}</p>
                        </div>
                        <div class="sm:col-span-2 lg:col-span-2">
                            <h5 class="text-[10px] font-extrabold text-slate-400 uppercase tracking-wider mb-0.5">{{ __('messages.address') }}</h5>
                            <p class="font-bold text-slate-900 text-xs leading-snug">{{ $member->memberProfile->address }}, {{ $member->memberProfile->city }} - {{ $member->memberProfile->pincode }}, {{ $member->memberProfile->state }}</p>
                        </div>
                    </div>
                @endif
    ...
    <div class="space-y-2.5">
        ...
        <div class="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-3">
            @forelse($member->familyMembers as $fam)
                <div class="bg-white rounded-xl p-3 border border-slate-200/80 shadow-xs space-y-2 hover:border-slate-300 transition-all">
                    ...
                    <div class="flex items-start justify-between gap-1.5 border-b border-slate-100 pb-1.5">
                        ...
                        <span class="px-2 py-0.5 rounded-md bg-primary-50 text-primary-700 border border-primary-100/80 text-[9px] font-extrabold uppercase shrink-0">
                            {{ $fam->relationship }}
                        </span>
                    </div>
                    ...
                    <div class="space-y-1 text-xs">
                        <div class="flex items-center justify-between gap-2">
                            <span class="text-[9px] font-extrabold text-slate-400 uppercase tracking-wider">{{ __('messages.blood_group') }}</span>
                            <span class="font-bold text-slate-800 text-[10px] truncate">{{ $fam->blood_group ?? __('messages.not_declared') }}</span>
                        </div>
                        <div class="flex items-center justify-between gap-2">
                            <span class="text-[9px] font-extrabold text-slate-400 uppercase tracking-wider">{{ __('messages.mobile') }}</span>
                            <span class="font-bold text-slate-800 text-[10px] truncate max-w-[110px]">{{ $fam->phone ?? __('messages.not_declared') }}</span>
                        </div>
                        <div class="flex items-center justify-between gap-2">
                            <span class="text-[9px] font-extrabold text-slate-400 uppercase tracking-wider">{{ __('messages.email') }}</span>
                            <span class="font-bold text-slate-800 text-[10px] truncate max-w-[120px]">{{ $fam->email ?? __('messages.not_declared') }}</span>
                        </div>
                        <div class="flex items-center justify-between gap-2">
                            <span class="text-[9px] font-extrabold text-slate-400 uppercase tracking-wider">{{ __('messages.education') }}</span>
                            <span class="font-bold text-slate-800 text-[10px] truncate max-w-[110px]">{{ $fam->education ?? __('messages.not_declared') }}</span>
                        </div>
                        <div class="flex items-center justify-between gap-2">
                            <span class="text-[9px] font-extrabold text-slate-400 uppercase tracking-wider">{{ __('messages.occupation') }}</span>
                            <span class="font-bold text-slate-800 text-[10px] truncate max-w-[110px]">{{ $fam->occupation ?? __('messages.not_declared') }}</span>
                        </div>
                    </div>

                </div>
            @empty
                <div class="col-span-full bg-white rounded-xl p-5 text-center text-slate-400 border border-slate-200/80 text-xs font-semibold">
