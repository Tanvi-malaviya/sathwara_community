...
<div x-data="{
    ...
    <div class="flex flex-col sm:flex-row sm:items-center justify-between gap-3 bg-white p-3 rounded-xl border border-slate-100 shadow-sm mb-4">
        <form method="GET" action="{{ route('admin.content.sliders') }}" class="flex items-center gap-2 flex-grow max-w-md w-full">
            <div class="relative flex-grow">
                <input type="text" name="search" value="{{ request('search') }}" placeholder="{{ __('messages.search_sliders') }}" 
                ...
                @if(request()->filled('search'))
                    <a href="{{ route('admin.content.sliders') }}" class="absolute right-2.5 top-1/2 -translate-y-1/2 text-slate-400 hover:text-rose-500 font-extrabold text-sm" title="Clear search">
                        &times;
                    </a>
                @endif
            </div>
        ...
        <button @click="showAddModal = true" class="inline-flex items-center justify-center px-4 py-2 bg-primary-500 hover:bg-primary-600 font-bold text-xs text-white rounded-xl shadow-md transition-all hover:scale-[1.02] active:scale-95 shrink-0 whitespace-nowrap">
    ...
    <div class="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-2">
        @forelse($sliders as $slide)
            <div class="bg-white border border-slate-100 rounded-2xl overflow-hidden shadow-sm flex flex-col justify-between hover:shadow-md transition-shadow">
                <div class="aspect-video w-full overflow-hidden bg-slate-50 relative">
                    <img class="w-full h-full object-cover"
                         ...
                         alt="Slider image">
                    <span class="absolute top-2 right-2 px-2 py-0.5 text-[9px] font-bold rounded {{ $slide->status ? 'bg-emerald-500 text-white' : 'bg-slate-400 text-white' }}">
        ...
        @empty
            <div class="col-span-full text-center py-16 text-slate-400 bg-white rounded-3xl border border-slate-100 shadow-sm space-y-3">
    ...
    <div x-show="showAddModal" class="fixed inset-0 z-50 flex items-center justify-center p-4 bg-slate-900/60 backdrop-blur-sm" x-transition x-cloak>
        <div @click.away="showAddModal = false" class="bg-white rounded-2xl p-4 border border-slate-100 shadow-2xl max-w-sm w-full space-y-3 relative max-h-[90vh] overflow-y-auto">
            ...
            <form method="POST" action="{{ route('admin.content.sliders.store') }}" enctype="multipart/form-data" class="space-y-3">
                @csrf
                ...
                <div class="grid grid-cols-2 gap-3">
                    <div class="space-y-0.5">
                        ...
                        <input type="text" name="button_text" placeholder="View Details" class="w-full text-xs px-3 py-1.5 bg-slate-50 border border-slate-200 rounded-lg">
                    </div>
                    <div class="space-y-0.5">
                        <label class="text-[10px] font-bold text-slate-400 uppercase tracking-wide">{{ __('messages.button_link') }}</label>
                        <input type="text" name="button_link" placeholder="/events" class="w-full text-xs px-3 py-1.5 bg-slate-50 border border-slate-200 rounded-lg">
                    </div>
                </div>
                <div class="grid grid-cols-2 gap-3">
                    <div class="space-y-0.5">
                    ...
                    <div class="space-y-0.5">
                        <label class="text-[10px] font-bold text-slate-400 uppercase tracking-wide">{{ __('messages.order') }}</label>
                        <input type="number" name="display_order" value="0" required class="w-full text-xs px-3 py-1.5 bg-slate-50 border border-slate-200 rounded-lg">
                    </div>
                </div>
                <div class="pt-2 border-t border-slate-100 flex justify-end gap-2">
                    ...
                    <button type="submit" class="px-4 py-1.5 bg-primary-500 hover:bg-primary-600 text-white font-bold text-xs rounded-lg shadow-sm">{{ __('messages.add_slider_submit') }}</button>
                </div>
            </form>
        </div>
    </div>
        ...
        <div @click.away="showEditModal = false" class="bg-white rounded-2xl p-4 border border-slate-100 shadow-2xl max-w-sm w-full space-y-3 relative max-h-[90vh] overflow-y-auto">
            ...
            <form method="POST" :action="editSlider.update_url" enctype="multipart/form-data" class="space-y-3">
                ...
                <div class="grid grid-cols-2 gap-3">
                    <div class="space-y-0.5">
                        <label class="text-[10px] font-bold text-slate-400 uppercase tracking-wide">{{ __('messages.button_text') }}</label>
                        <input type="text" name="button_text" :value="editSlider.button_text" placeholder="View Details" class="w-full text-xs px-3 py-1.5 bg-slate-50 border border-slate-200 rounded-lg">
                    </div>
                    <div class="space-y-0.5">
                        <label class="text-[10px] font-bold text-slate-400 uppercase tracking-wide">{{ __('messages.button_link') }}</label>
                        <input type="text" name="button_link" :value="editSlider.button_link" placeholder="/events" class="w-full text-xs px-3 py-1.5 bg-slate-50 border border-slate-200 rounded-lg">
                    </div>
                </div>
                <div class="grid grid-cols-2 gap-3">
                    <div class="space-y-0.5">
                    ...
                    <div class="space-y-0.5">
                        <label class="text-[10px] font-bold text-slate-400 uppercase tracking-wide">{{ __('messages.order') }}</label>
                        <input type="number" name="display_order" :value="editSlider.display_order" required class="w-full text-xs px-3 py-1.5 bg-slate-50 border border-slate-200 rounded-lg">
                    </div>
                </div>
                <div class="pt-2 border-t border-slate-100 flex justify-end gap-2">
