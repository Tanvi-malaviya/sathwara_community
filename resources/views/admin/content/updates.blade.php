...
@section('content')
    <div x-data="{
        ...
        <div
            ...
            <form method="GET" action="{{ route('admin.content.updates') }}"
                class="flex items-center gap-2 flex-grow max-w-md w-full">
                <div class="relative flex-grow">
                    <input type="text" name="search" value="{{ request('search') }}"
                        placeholder="{{ __('messages.search_announcements') }}"
            ...
            <div class="flex items-center gap-2 shrink-0">
                <a href="{{ route('admin.content.updates.export', request()->all()) }}"
                    ...
                    @forelse($updates as $up)
                        <tr class="hover:bg-slate-50/50">
                            ...
                            <td class="py-2 px-3 text-center">
                                <span
                                    class="px-2 py-0.5 text-[9px] font-bold rounded uppercase {{ $up->status == 'published' ? 'bg-emerald-50 text-emerald-700' : 'bg-slate-100 text-slate-500' }}">
                                    {{ $up->status }}
                                </span>
                            </td>
            ...
            <div @click.away="showAddModal = false"
                ...
                <form method="POST" action="{{ route('admin.content.updates.store') }}" enctype="multipart/form-data"
                    ...
                    <div class="space-y-0.5">
                        <label
                            class="text-[10px] font-bold text-slate-400 uppercase tracking-wide">{{ __('messages.headline_title') }}</label>
