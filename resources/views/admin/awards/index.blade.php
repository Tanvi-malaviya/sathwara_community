    ...
    <div class="space-y-4">
        ...
        <div class="bg-white p-3 rounded-xl border border-slate-100 shadow-sm">
            <div class="flex flex-col lg:flex-row lg:items-center justify-between gap-3">
                <!-- Search bar -->
                <form method="GET" action="{{ route('admin.awards.index') }}"
                    ...
                    <div class="relative flex-grow">
                        <input type="text" name="search" value="{{ request('search') }}"
                            placeholder="{{ __('messages.search_awards_admin') }}"
                        ...
                        @if(request()->filled('search'))
                            <a href="{{ route('admin.awards.index', request()->except('search')) }}"
                    ...
                    <a href="{{ route('admin.awards.export', request()->all()) }}"
                        class="px-3.5 py-2 bg-emerald-50 hover:bg-emerald-100 text-emerald-700 font-bold text-xs rounded-xl border border-emerald-200/60 shadow-xs transition-colors shrink-0 whitespace-nowrap inline-flex items-center gap-1.5">
                        ...
                        <tr class="hover:bg-slate-50/50" x-data="{ showNotes: false }">
                            ...
                            <td class="py-2.5 px-4">
                                <div class="min-w-0">
                                    ...
                                    <span class="text-[10px] text-slate-400 font-medium">{{ $app->standard }} •
                                        {{ $app->school }}</span>
                                </div>
                            </td>
                            <td class="py-2.5 px-4">
                            ...
                            <td class="py-2.5 px-4">
                                <a href="{{ asset('storage/' . $app->certificate_path) }}" target="_blank"
                            ...
                            <td class="py-2.5 px-4 text-right">
                                <div class="flex flex-col items-end gap-2">
                                    <div class="flex justify-end items-center space-x-2">
                                        @if($app->status == 'pending')
                                            <form method="POST" action="{{ route('admin.awards.approve', $app->id) }}">
                                                @csrf
                                                <button type="submit"
                                                    class="flex items-center justify-center w-8 h-8 rounded-lg bg-emerald-50 text-emerald-700 hover:bg-emerald-100 transition-colors"
                                            ...
                                            <button @click="showNotes = !showNotes" type="button"
                                                class="flex items-center justify-center w-8 h-8 rounded-lg bg-rose-50 text-rose-700 hover:bg-rose-100 transition-colors"
                                        ...
                                        @endif
                                        <button type="button"
                                            @click="$dispatch('confirm-delete', { action: '{{ route('admin.awards.destroy', $app->id) }}', message: '{{ __('messages.delete_confirm_award', ['name' => $app->student_name]) }}' })"
                                            class="flex items-center justify-center w-8 h-8 rounded-lg bg-slate-100 text-slate-400 hover:text-slate-650 hover:bg-slate-200 transition-colors"
                                    ...
                                    <div x-show="showNotes" class="mt-2 text-left" x-cloak>
                                        <form method="POST" action="{{ route('admin.awards.reject', $app->id) }}"
                                            class="flex gap-2">
                                            @csrf
                                            <input type="text" name="admin_notes" required
                                                placeholder="{{ __('messages.rejection_reason_placeholder') }}"
                                                class="text-[10px] font-medium px-2 py-1 border border-slate-200 rounded-lg w-40">
                                            <button type="submit"
                                                class="px-2 py-1 bg-slate-900 text-white text-[9px] font-bold rounded-lg">{{ __('messages.save') }}</button>
                                        </form>
                                    </div>
                                </div>
                            </td>
