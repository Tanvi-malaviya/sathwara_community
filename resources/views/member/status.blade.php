...
<head>
    ...
    <title>{{ __('messages.account_status') }} - {{ App\Models\Setting::get('website_name', 'Sathwara Portal') }}</title>
    <!-- Google Fonts -->
...
<body class="bg-slate-50 text-slate-800 antialiased min-h-screen flex items-center justify-center p-6">
    <div class="max-w-md w-full bg-white rounded-3xl border border-slate-100 p-8 text-center space-y-6 shadow-sm">
        ...
        <div class="space-y-2">
            ...
            <p class="text-xs text-slate-400 font-semibold">{{ __('messages.registered_label') }}: {{ $user->created_at->format('d M, Y') }}</p>
        </div>

        @if($user->status === 'pending')
            <div class="p-5 rounded-2xl bg-amber-50 border border-amber-100 text-amber-800 text-xs font-semibold leading-relaxed space-y-3">
                <span class="text-2xl block">⏳</span>
                <h3 class="font-extrabold text-sm">{{ __('messages.app_pending_approval') }}</h3>
                <p>{{ __('messages.app_pending_desc') }}</p>
            </div>
        @elseif($user->status === 'rejected')
            <div class="p-5 rounded-2xl bg-rose-50 border border-rose-100 text-rose-800 text-xs font-semibold leading-relaxed space-y-3">
                <span class="text-2xl block">❌</span>
        ...
        <div class="pt-4 border-t border-slate-100 flex justify-between items-center">
            <a href="{{ route('home') }}" class="text-xs font-bold text-slate-500 hover:text-slate-900">&larr; {{ __('messages.back_to_website') }}</a>
            
            <form method="POST" action="{{ route('logout') }}">
                @csrf
                <button type="submit" class="text-xs font-bold text-rose-600 hover:text-rose-700">{{ __('messages.sign_out') }} &rarr;</button>
            </form>
        </div>
    </div>
</body>
