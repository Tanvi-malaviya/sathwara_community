...
<div x-data="{ 
     ...
     @confirm-delete.window="triggerDelete($event.detail.action, $event.detail.message)"
     x-show="open"  
     class="fixed inset-0 z-[100] flex items-center justify-center p-4 bg-slate-900/60 backdrop-blur-sm"
     x-transition
     x-cloak>
     <div class="bg-white rounded-3xl p-6 max-w-sm w-full border border-slate-100 shadow-2xl space-y-4" @click.away="open = false">
         ...
         <div class="flex justify-end items-center space-x-3 pt-2">
             <button type="button" @click="open = false" class="px-4 py-2 border border-slate-200 text-slate-700 font-bold text-xs rounded-xl hover:bg-slate-50 transition-colors">
                 {{ __('messages.cancel') }}
             </button>
             <form :action="formAction" method="POST">
                 @csrf
                 @method('DELETE')
                 <button type="submit" class="px-4 py-2 bg-rose-600 hover:bg-rose-700 text-white font-bold text-xs rounded-xl shadow-sm transition-colors">
                     {{ __('messages.delete') }}
                 </button>
             </form>
         </div>
